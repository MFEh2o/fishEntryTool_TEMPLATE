...
updateMinnowTrap<-function(headerRows=17,dbdir="C:/Users/Amary/Documents/MFEDB/database/",db="MFEdb.db",funcdir="C:/Users/Amary/Documents/MFEDB/fishEntryTool/",
                     force_lakeID=FALSE,
                     force_siteID=FALSE,
  ...
  #toCompile=toCompile[grepl("minnowtrap",toCompile)] -- this is not applicable to my file sorting
  ...
  if(length(toCompile)==0){
    ...
    for(i in 1:length(toCompile)){
      ...
      if(any(headerVals=="",na.rm=TRUE)){
        headerText=c("projectID","lakeID","dateTimeSet","dateTimeSample","crew","gear","distanceShocked","effort","effortUnits","comments","dataRecorder","dataEnteredBy","metadataID","useSampleMarkRecap","sampleGroup")
      ...
      curData$trapNumber=gsub("MT","",curData$trapNumber) # added in so that trapNumber can still become numeric in the next line
      curData$trapNumber=as.numeric(curData$trapNumber)
      
      # generate site id based on trapNumber
      curData$siteID=lakeID
      curData$siteID[curData$trapNumber<10]=paste(lakeID,"_MT.00",curData$trapNumber[curData$trapNumber<10],sep="")
      curData$siteID[curData$trapNumber>9 & curData$trapNumber<100]=paste(lakeID,"_MT.0",curData$trapNumber[curData$trapNumber>9 & curData$trapNumber<100],sep="")
      curData$siteID[curData$trapNumber>99]=paste(lakeID,"_MT.",curData$trapNumber[curData$trapNumber>99],sep="")
      ...
      if(sum(toFI$species!="NFC")!=0){
        fishInfoNEW=data.frame(projectID=rep(projectID,nrow(toFI)),
                            ...
                            clipRecapture=if("clipRecap"%in%colnames(toFI)) toFI$clipRecap else NA,
                            ...
                            otolithSampled=if("otolithSampled"%in%colnames(toFI)) toFI$otolithSampled else NA,
                            ...
                            gonadSqueeze=if("gonadSqueeze"%in%colnames(toFI)) toFI$gonadSqueeze else NA,
                            ...
                            spineSample=if("spineSampled"%in%colnames(toFI)) toFI$spineSampled else NA,
                            scaleSample=if("scaleSampled"%in%colnames(toFI)) toFI$scaleSampled else NA,
                            comments=if("comment"%in%colnames(toFI)) toFI$comment else NA,
        ...
        for(j in 1:length(abbrevs)){
          #if(abbrevs[j] == "NFC"){
           # fishInfoNEW$species[fishInfoNEW$species==abbrevs[j]]="none"
          #}
           ...
           if(nrow(curDB)>15){
             ...
             if(any(curNEW$fishWeight<predsLow | curNEW$fishWeight>predsHigh)){
               if(force_fishWeight==FALSE){
                 stop(paste("You report fishWeight outside the prediction based on a length-weight regression from our database for",uniqSpec[j],"in sample",paste(lakeID,dateSampleString,timeSampleString,gear,metadataID,sep="_"),"if you are certain you have fishWeight correct for all individuals use the argument force_fishWeight;",
